...
public class ImpegnativaOrtopedico extends JFrame {
    ...
    private void loadServicesByCodiceFiscale(String codiceFiscale) {
        try {
			List<Impegnativa> servizio = controller.searchByCodiceFiscale(codiceFiscale);
            tableModel.setRowCount(0);
            for (Impegnativa servizi : servizio) {
    ...
    private void saveAllRowsToDatabase() {
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            if (validateFields(prezzo, descrizione, trattamento, codiceFiscale)) {
				...
				Impegnativa servizi = new Impegnativa(prezzo, descrizione, trattamento, codiceFiscale, "Ortopedico");
                try {
    ...
    public static void main(String[] args) {
        ...
        frame.add(new ImpegnativaOrtopedico()); // Sostituisci con un'email valida
