...
public class InventarioMaterialiOrtopedico extends JFrame {
    ...
    public InventarioMaterialiOrtopedico() {
        ...
        buttonAddRow.addActionListener(_ -> tableModel.addRow(new Object[]{"", "", "", "", ""}));

        // Elimina il materiale selezionato
        buttonDeleteMaterial.addActionListener(_ -> deleteSelectedRow());

        // Ordina il materiale selezionato
        buttonOrderMaterial.addActionListener(_ -> orderMaterial());

        // Salva l'ultima riga nel database
        buttonSave.addActionListener(_ -> saveToDatabase());
    }

    private void loadDataFromDatabase() {
        try {
            List<InventarioMateriale> materiali = controller.getAllMateriali();
    ...
    private void deleteSelectedRow() {
        ...
        if (selectedRow != -1) {
            ...
            try {
            	controller.deleteMateriale(nomeMateriale);
    ...
    private void orderMaterial() {
        ...
        if (selectedRow != -1) {
            String nomeMateriale = (String) tableModel.getValueAt(selectedRow, 0);
            try {
            	controller.orderMateriale(nomeMateriale);
    ...
    private void saveToDatabase() {
        ...
        try {
        	controller.addMateriale(materiale);
