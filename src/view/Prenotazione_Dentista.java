...
public class Prenotazione_Dentista extends JFrame {
	...
	public Prenotazione_Dentista() {
        this.setTitle("Prenotazione Dentista");
    ...
    private void addToDatabase() {
        controller.setTipoDiUtente("DENTISTA"); // Imposta il tipo di utente
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            Prenotazione prenotazione = new Prenotazione(cognome, nome, codiceFiscale, data, ora, "DENTISTA");
            try {
                controller.aggiungiPrenotazione(prenotazione); // Chiama il controller per aggiungere la prenotazione
                JOptionPane.showMessageDialog(this, "Prenotazione salvata con successo!", "Successo", JOptionPane.INFORMATION_MESSAGE);
            } catch (SQLException ex) {
